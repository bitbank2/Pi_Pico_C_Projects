...
{
	gpio_put(pLCD->iDCPin, iMode == MODE_DATA);
	sleep_us(1); // some systems are so fast that it needs to be delayed
} /* spilcdSetMode() */
...
const unsigned char ucSSD1331InitList[]  = {
    1, 0xae,    // display off
    1, 0xa0,    // set remap
    1, 0x72,    // RGB 0x76 == BGR
...
const unsigned char ucOLEDInitList[] = {
...
const unsigned char uc132InitList[] = {
    3, 0x10, 0x2F,0x8E,
    3, 0x11, 0x00,0x0C,
    3, 0x07, 0x00,0x21,
    3, 0x28, 0x00,0x06,
...
const unsigned char uc320InitList[] = {
        2, 0xc0, 0x23, // Power control
        2, 0xc1, 0x10, // Power control
        3, 0xc5, 0x3e, 0x28, // VCM control
        2, 0xc7, 0x86, // VCM control2
...
const unsigned char ucILI9225InitList[] = {
...
const unsigned char uc128InitList[] = {
//	4, 0xb1, 0x01, 0x2c, 0x2d,	// frame rate control
//	4, 0xb2, 0x01, 0x2c, 0x2d,	// frame rate control (idle mode)
//	7, 0xb3, 0x01, 0x2c, 0x2d, 0x01, 0x2c, 0x2d, // frctrl - partial mode
//	2, 0xb4, 0x07,	// non-inverted
...
const unsigned char ucILI9486InitList[]  = {
   2, 0x01, 0x00,
   LCD_DELAY, 50,
   2, 0x28, 0x00,
   3, 0xc0, 0xd, 0xd,
...
const unsigned char uc480InitList[] = {
	2, 0x3a, 0x55,
	2, 0xc2, 0x44,
...
//        if (pLCD->iCSPin != -1)
//            gpio_put(pLCD->iCSPin, HIGH);
        return;
    }
#endif // HAS_DMA
        
// No DMA requested or available, fall through to here
    spi_write_blocking(SPI_INST, pBuf, iLen);
...
int i, iCount;
  ...
  if (pLCD->pfnResetCallback != NULL)
  {
    ...
    pLCD->iSPIMode = (iType == LCD_ST7789_NOCS || iType == LCD_ST7789) ? 3 : 0;
    ...
    pLCD->iLEDPin = iLED;
    pLCD->iMOSIPin = iMOSIPin;
    pLCD->iCLKPin = iCLKPin;
    ...
    {
        gpio_init(pLCD->iMOSIPin);
        gpio_set_dir(pLCD->iMOSIPin, GPIO_OUT);
        gpio_init(pLCD->iCLKPin);
        gpio_set_dir(pLCD->iCLKPin, GPIO_OUT);
        goto skip_spi_init;
    }
    else
    {
       spi_init(SPI_INST, iSPIFreq);
//       if (pLCD->iSPIMode == 3)
...
//           spi_set_format(SPI_INST, 8, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
       gpio_set_function(iDC, GPIO_FUNC_SIO);
       gpio_set_dir(iDC, GPIO_OUT);

       gpio_set_function(iCS, GPIO_FUNC_SIO);
       gpio_set_dir(iCS, GPIO_OUT);

       gpio_set_function(pLCD->iCLKPin, GPIO_FUNC_SPI);
       gpio_set_function(pLCD->iMOSIPin, GPIO_FUNC_SPI);
       if (iMISOPin != -1) {
          gpio_set_function(iMISOPin, GPIO_FUNC_SPI);
       }
       // Set up DMA
       dma_tx = dma_claim_unused_channel(1);
       dma_ch_config = dma_channel_get_default_config(dma_tx);
       channel_config_set_transfer_data_size(&dma_ch_config, DMA_SIZE_8);
       channel_config_set_dreq(&dma_ch_config, spi_get_index(SPI_INST) ? DREQ_SPI1_TX : DREQ_SPI0_TX);
...
//
    ...
    if (pLCD->iResetPin != -1)
    {
        gpio_set_function(pLCD->iResetPin, GPIO_FUNC_SIO);
        gpio_set_dir(pLCD->iResetPin, GPIO_OUT);
        gpio_put(pLCD->iResetPin, HIGH);
        sleep_us(60000);
        gpio_put(pLCD->iResetPin, LOW); // reset the controller
        sleep_us(60000);
        gpio_put(pLCD->iResetPin, HIGH);
        sleep_us(60000);
    }
...
start_of_init:
    ...
    else if (pLCD->iLCDType == LCD_SSD1331)
    {
        ...
        memcpy(d, ucSSD1331InitList, sizeof(ucSSD1331InitList));
        s = d;
	...
	else if (pLCD->iLCDType == LCD_SSD1351)
	{
		s = (unsigned char *)ucOLEDInitList; // do the commands manually
                memcpy(d, s, sizeof(ucOLEDInitList));
    ...
    else if (pLCD->iLCDType == LCD_SSD1283A)
    {
        ...
        memcpy(d, s, sizeof(uc132InitList));
...
void spilcdShutdown(SPILCD *pLCD)
{
	...
	if (pLCD->iLEDPin != -1)
		gpio_put(pLCD->iLEDPin, 0); // turn off the backlight
...
{
#ifdef HAS_DMA
    dma_channel_configure(dma_tx, &dma_ch_config,
                          &spi_get_hw(SPI_INST)->dr, // write address
                          ucTXBuf, // read address
                          iLen, // element count (each element is of size transfer_data_size)
                          1); // start immediately
...
uint16_t *d, u16Temp[TEMP_BUF_SIZE];
   ...
   while (szMsg[i] && x < pLCD->iCurrentWidth)
   {
      ...
      if (bBlank) { // erase the areas around the char to not leave old bits
         int miny, maxy;
         c = '0' - font.first;
         miny = y + *(int8_t *)(&font.glyph[c].yOffset);
         c = 'y' - font.first;
         maxy = y + *(int8_t *)(&font.glyph[c].yOffset) + *(uint8_t *)(&font.glyph[c].height);
